...
Module Application_Code
    ...
    Sub Main(ByVal sArgs() As String)
        Try
            If sArgs.Length < 2 And sArgs.Length > 4 Then
                Console.WriteLine("Registry Key Creator")
                Console.WriteLine("-------------------------")
                Console.WriteLine("Usage: executable [RegistryRoot] [RegistryKey] {RegistryValue} {ValueData}")
                Console.WriteLine("  where:")
                Console.WriteLine("   - [RegistryRoot]: the registry root key, e.g. HKEY_LOCAL_MACHINE")
                Console.WriteLine("   - [RegistryKey]: the actual fully qualified key to create minus the root key, e.g. SOFTWARE\Microsoft")
                Console.WriteLine("   - {RegistryValue}: Optional Registry Value name to be created")
                Console.WriteLine("   - {ValueData}: Optional Registry Value Data to be set")
            Else
                If sArgs.Length = 2 Then
                    Console.WriteLine(CreateSubRegKey(sArgs(0), sArgs(1), "", ""))
                End If
                If sArgs.Length = 3 Then
                    Console.WriteLine(CreateSubRegKey(sArgs(0), sArgs(1), sArgs(2), ""))
                End If
                If sArgs.Length = 4 Then
                    Console.WriteLine(CreateSubRegKey(sArgs(0), sArgs(1), sArgs(2), sArgs(3)))
                End If
                End If
    ...
    Private Sub Error_Handler(ByVal ex As Exception, Optional ByVal identifier_msg As String = "")
        ...
        Catch exc As Exception
            Console.WriteLine("An error occurred in Registry Key Creator's error handling routine. The application will try to recover from this serious error.", MsgBoxStyle.Critical, "Critical Error Encountered")
        End Try
    End Sub
    ...
    Public Function CreateSubRegKey(ByVal MainKey As String, ByVal RequestedKey As String, Optional ByVal Value As String = "", Optional ByVal Data As String = "") As String
        ...
        Try
            ...
            Try
                ...
                End If
            Catch ex As Exception
                Error_Handler(ex, "CreateSubRegKey")
